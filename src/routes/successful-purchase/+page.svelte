<script lang="ts">
	...
	let { sessionData }: { sessionData: any } = $derived(data);
	...
	// 		// Handle error - maybe fallback to direct navigation
	...
	// 		const newWindow = window.open(pdfUrl, `invoice-${invoiceNumber}`);

	// 		// If window.open was blocked or failed, fall back to direct navigation
	// 		if (!newWindow) {
	// 			window.location.href = pdfUrl;
	// 		}
	// 	} catch (error) {
	...
	>
		...
		<div class="lm:gap-8 lm:p-8 flex w-full flex-col gap-4 p-4">
			...
			<button
				...
				focus-within:bg-surface-primary-700 flex w-full cursor-pointer items-center
				justify-center gap-3 rounded-md border
			px-2 py-5 text-xl font-medium uppercase
