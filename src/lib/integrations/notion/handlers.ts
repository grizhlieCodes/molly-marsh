...
import { 
  ...
  createSessionInNotion,
  createInvoiceInNotion
} from './creators';
import {
  cancelSessionInNotion,
  updateSessionTimeDateInNotion,
  updateSessionStatusBasedOnAttendeesInNotion
} from './updaters';
...
export async function handleMeetingEnded(calData, event: RequestEvent) {
  try {
    ...
    const updatedSession = await updateSessionStatusBasedOnAttendeesInNotion(
      calSession.id, 
      zoomAttendees
    );
