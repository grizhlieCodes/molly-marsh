...
<label for="{blok.input_name}" class="{labelCss} group" use:storyblokEditable={blok}>
	...
	group-focus-within:text-molly-dark-600 has-[+*:not(:placeholder-shown)]:-translate-x-[0.6rem]
	...
	<textarea
		...
		class="h-[8rem] max-h-[3rem] w-full rounded-md border-b
            border-molly-dark-300 bg-molly-dark-50 px-3 py-2.5
	outline-none transition-all duration-500 focus:border-molly-dark-500
	focus:bg-molly-dark-100/70  group-focus-within:max-h-[10rem]
	overflow-hidden focus:overflow-scroll text-molly-dark-500 group-focus-within:text-neutral-800
    "
		placeholder=" "
		rows="4"
		bind:value={formBind}
